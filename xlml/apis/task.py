...
class XpkTask(BaseTask):
  ...
  interruption_timeout: datetime.timedelta = datetime.timedelta(
      minutes=1
  )
  ...
  def run_check_restore(
    ...
    """
    with TaskGroup(group_id=self.task_test_config.benchmark_id) as group:
      run_model, gcs_path = self.run_model_restore_validation(
          gcs_location,
          use_vertex_tensorboard,
          use_pathways,
          ramdisk_directory,
  ...
  def run_model_restore_validation(
      self,
      gcs_location: Optional[airflow.XComArg] = None,
      use_vertex_tensorboard: bool = False,
      use_pathways: bool = False,
    ...
    with TaskGroup(group_id="run_model") as group:
      ...
      launch_workload = self.launch_workload_with_restore_validation(
          workload_id,
          gcs_path,
          use_vertex_tensorboard,
          use_pathways,
  ...
  def launch_workload_with_restore_validation(
      self,
      workload_id: str,
      gcs_path: str,
      use_vertex_tensorboard: bool,
    ...
    """
    Create a workload then first interrupt the workload pod,
    validate the EMC is restoring from the GCS Bucket
    """
    with TaskGroup(group_id="launch_workload_with_interruption_and_restore_validation") as group:
      run_workload = xpk.run_workload.override(
          owner=self.task_test_config.task_owner
      )(
          task_id="run_workload_with_restore",
          cluster_project=self.task_gcp_config.project_name,
          zone=self.task_gcp_config.zone,
          cluster_name=self.task_test_config.cluster_name,
          benchmark_id=self.task_test_config.benchmark_id,
      ...
      run_restore_validation = xpk.check_restoring_from_gcs_or_local.override(
          owner=self.task_test_config.task_owner
      )(
          task_id="run_restore_validation",
          project_id=self.task_gcp_config.project_name,
          region=self.task_gcp_config.zone[:-2],
          cluster_name=self.task_test_config.cluster_name,
          workload_id=workload_id,
      ...
      run_workload >> wait_for_workload_start >> run_restore_validation
      return group
