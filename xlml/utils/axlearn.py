...
def set_up_axlearn(tmpdir,branch) -> Tuple[str]:
  if branch != "":
    logging.log("Using Lalaitah's branch")
    clone_branch = (
        f"git clone --branch {branch} git@github.com:lkolluru05/axlearn.git"
        f" {tmpdir}/axlearn"
    )
  cmds = [
      "set -xue",
      clone_branch,
      f"cd {tmpdir}/axlearn/ && pip install -e '.[core,dev]' pip install -e '.[gcp]' ",
      UPGRADE_PIP,
      UPGRADE_SETUPTOOLS,
      UPGRADE_PACKAGING,
...
def run_workload(
  ...
  with tempfile.TemporaryDirectory() as tmpdir:
    ...
    workload_create_cmd = (
        f"axlearn gcp launch  --cluster={cluster_name}  --runner_name gke_tpu_single "
        f" --name={run_name}  --instance_type={accelerator_type} "
        f" --num_replicas={num_replicas}   --bundler_spec=allow_dirty=True "
        f" --bundler_type=artifactregistry --bundler_spec=image=tpu  --bundler_spec=dockerfile=Dockerfile "
        f" --bundler_spec=target=tpu -- '{run_cmds}' "
