...
def set_up_axlearn(tmpdir,branch)-> Tuple[str]:
  ...
  export_var = [
    "export CLUSTER=${CLUSTER:-camiloquinones-axlearn}",
    f"export NAME=camiloquinones-lal",
  ...
  echo_command_string = f"echo -e \"{config_content}\" > ~/axlearn/.axlearn/axlearn.default.config"
  create_axlearn_conf = [echo_command_string]
  ...
  cmds = [
      ...
      *install_python3_cmd,
      ...
      f"cd ~/axlearn/ ",
      *hack_dpndcies_cmd,
      f"/home/airflow/.pyenv/versions/3.10.12/bin/python -m pip  install -e '.[core,gcp,dev]'",
      "pip list",
      "pyenv rehash",
      "which axlearn",
      *create_axlearn_conf,
...
def run_workload_axlearn(
    ...
    workload_id: str,
    gcs_path: str,
    accelerator_type: str="tpu-v5p-128",
    run_cmds: str="",
    module: str="",
    model_config: str="",
    trainer_dir: str="",
  ...
  with tempfile.TemporaryDirectory() as tmpdir:
    run_cmds = "ulimit -n 1048576; ulimit -c 0; python3 -c 'import jax; jax.devices()'; python3 -m axlearn.common.launch_trainer_main"
    cmds = set_up_axlearn(tmpdir, LALITAH_BRANCH)
    # core_api = _get_core_api_client(project_id, region, cluster_name)
    # pods = _list_workload_pods(core_api, workload_id)

    workload_create_cmd = (
        f" export TPU_TYPE=\"tpu-v5p-128\" &&  export NUM_TPU_SLICES=4 && /home/airflow/.pyenv/versions/3.10.12/bin/python -m axlearn.common.launch_trainer_main"
        f" --module={module}  --config={model_config} "
        f" --trainer_dir={trainer_dir}  --data_dir=gs://axlearn-public/tensorflow_datasets "
        f" --jax_backend=tpu  --mesh_selector={accelerator_type} "
        f" --initialization_timeout=1200 "
        f" --trace_at_steps=49,99,149 "
    )
    ...
    result = hook.run_command(
        ["bash", "-c", ";".join(cmds)],
        env={**os.environ, "KUBECONFIG": os.path.join(tmpdir, "axlearn_var.conf")},
        )
