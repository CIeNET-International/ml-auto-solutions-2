...
CREATE OR REPLACE VIEW `amy_xlml_poc_prod.dag_run_statistic_lastdays_7` AS

-- DAG runs within the specified date range
...
dag_runs AS (
    ...
    FROM dag_runs_ended
    WHERE execution_date BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 7 DAY) AND CURRENT_TIMESTAMP()
),
