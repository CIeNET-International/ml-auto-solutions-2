...
all_tests_pre AS (
  SELECT base.dag_id, runs.run_id,runs.execution_date, runs.start_date, runs.end_date, runs.is_passed, runs.run_order_desc, runs.is_passed_run_order_desc, FALSE AS is_quarantined_dag,
    tests.test_id,tests.start_date test_start_date, tests.end_date test_end_date, FALSE AS is_quarantined_test
  FROM `cienet-cmcs.amy_xlml_poc_prod.base` base
  JOIN UNNEST (base.runs) AS runs
...
all_tests_qr AS (
  SELECT base.dag_id, runs.run_id,runs.execution_date, runs.start_date, runs.end_date, runs.is_passed, runs.run_order_desc, runs.is_passed_run_order_desc, TRUE AS is_quarantined_dag,
    tests.test_id,tests.start_date test_start_date, tests.end_date test_end_date, TRUE AS is_quarantined_test
  FROM `cienet-cmcs.amy_xlml_poc_prod.base` base
  JOIN UNNEST (base.runs_qr) AS runs
...
all_tests AS (
  SELECT dag_id, run_id, execution_date, start_date, end_date, is_passed, run_order_desc, is_passed_run_order_desc, is_quarantined_dag, test_id, test_start_date, test_end_date, is_quarantined_test
  FROM all_tests_pre
  UNION ALL
  SELECT dag_id, run_id, execution_date, start_date, end_date, is_passed, run_order_desc, is_passed_run_order_desc, is_quarantined_dag, test_id, test_start_date, test_end_date, is_quarantined_test
  FROM all_tests_qr
