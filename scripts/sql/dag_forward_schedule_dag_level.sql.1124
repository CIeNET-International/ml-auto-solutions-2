CREATE OR REPLACE VIEW `amy_xlml_poc_prod.dag_forward_schedule_dag_level` AS
WITH dag_agg AS (
  SELECT
  ...
  FROM `amy_xlml_poc_prod.dashboard_dag_forward_schedule_simulation`
  GROUP BY cluster_name, project_name, region, dag_id, formatted_schedule, sim_date
),
concurrency AS (
  SELECT
    t.cluster_name, t.project_name, t.region,
    t.dag_id,
    t.formatted_schedule,
    DATE(t.test_sim_start_time) AS sim_date,
    MAX((
      SELECT COUNT(1)
      FROM `amy_xlml_poc_prod.dashboard_dag_forward_schedule_simulation` s
      WHERE s.cluster_name = t.cluster_name
        AND s.project_name = t.project_name
  ...
  FROM `amy_xlml_poc_prod.dashboard_dag_forward_schedule_simulation` t
  GROUP BY t.cluster_name, t.project_name, t.region, t.dag_id, t.formatted_schedule, sim_date
)
SELECT
...
FROM dag_agg a
JOIN concurrency c
  ON a.cluster_name = c.cluster_name AND a.project_name = c.project_name AND a.region = c.region
 AND a.dag_id = c.dag_id
 AND a.formatted_schedule = c.formatted_schedule
 AND a.sim_date = c.sim_date
JOIN `amy_xlml_poc_prod.dashboard_dag_forward_schedule_simulation` t
  ON a.cluster_name = t.cluster_name AND a.project_name = t.project_name AND a.region = t.region
