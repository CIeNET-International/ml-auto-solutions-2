...
last_run_tests_1 AS (
  SELECT base.dag_id, base.tags, runs.run_id, runs.execution_date, runs.start_date run_start_date, runs.end_date run_end_date, runs.is_passed, runs.run_order_desc, 
...
last_run_tests AS (
  SELECT * FROM last_run_tests_1
  UNION ALL
  SELECT * FROM last_run_tests_2
)
