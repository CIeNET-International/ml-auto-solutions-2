...
dag_tests_cluster AS (
  SELECT dag_id, test_id, cluster_name, project_name, region, accelerator_type, accelerator_family
...
t1_pre AS (
  ...
  ARRAY_AGG(STRUCT(d.dag_id, d.category, d.accelerator, d.total_runs, d.passed_runs, d.runs_pass_rate, 
    ...
    d.number_of_tests, min_run_start, max_run_end, d.total_run_tests, d.passed_tests, d.tests_pass_rate, d.formatted_schedule, d.is_paused, d.tags, d.total_runs_all, d.dag_owner, d.last_exec, d.last_succ)) AS dag_statistic 
...
t5 AS (
  SELECT c.window_name, ANY_VALUE(c.window_value) AS window_value,
    ARRAY_AGG(STRUCT(c.project_name, c.cluster_name, c.region, min_run_start, max_run_end, c.total_run_tests, c.passed_tests, c.tests_pass_rate)) AS cluster_statistic
