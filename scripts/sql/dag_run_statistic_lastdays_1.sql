...
CREATE OR REPLACE VIEW `amy_xlml_poc_prod.dag_run_statistic_lastdays_1` AS
...
dag_runs AS (
    ...
    FROM dag_runs_ended
    WHERE execution_date BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 1 DAY) AND CURRENT_TIMESTAMP()
),
