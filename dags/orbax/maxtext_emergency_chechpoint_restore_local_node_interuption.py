...
with models.DAG(
    dag_id="maxtext_emergency_res09_restore_local_node_interuption",
    schedule_interval=SCHEDULE,
    tags=[
        "multipod_team",
        "maxtext",
        "emergency_chkpt_restore_local_node_interuption",
        "nightly",
    ],
    start_date=datetime.datetime(2025, 7, 18),
    catchup=False,
    concurrency=2,
) as dag:
  base_output_directory = f"{gcs_bucket.MTC_AUTOMATION_BUCKET}/maxtext_emergency_res09_node_interuption"
  docker_images = [
      (
          SetupMode.NIGHTLY,
  ...
  name_prefix = f"maxtext_{model_name}_phase2_chkpt_restore"
