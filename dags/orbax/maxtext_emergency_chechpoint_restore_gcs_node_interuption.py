...
with models.DAG(
  ...
  base_output_directory = f"{gcs_bucket.MTC_AUTOMATION_BUCKET}/maxtext_multi_tier_res10_node_interuption"
  docker_images = [
      (
          SetupMode.NIGHTLY,
  ...
  for mode, image in docker_images:
    for accelerator, slices in test_configs.items():
      for slice_num in slices:
        ...
        validate_gcs_bucket_save_step = log_explorer.validate_log_exist(
            project_id=clusters[accelerator].project,
            location=clusters[accelerator].zone[:-2],
            cluster_name=clusters[accelerator].name,
            text_filter=f"Finished restoring checkpoint "
            f"in seconds from {base_output_directory}",
