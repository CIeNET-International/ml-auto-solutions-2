...
try:
  ...
  from dags.dashboard.statistic import save_log_utils as utils
except ImportError:
  # Fallback for standalone/venv environment
  from config import (
    BQ_PROJECT_ID, BQ_DATASET, 
    GCS_PROJECT_ID, GCS_BUCKET_NAME,
    LOG_PROJECT_ID
...
def parse_cluster_metadata(payload_string):
    ...
    #xlml_metadata {"cluster_project": "cienet-cmcs", "region": "us-central1", "zone": "us-central1-b", "cluster_name": "tpu-observability-automation-prod", "node_pool_name": "multi-host-nodepool-rollback-auto", "accelerator_type": "ct6e-standard-4t"}
    try:
        ...
        if metadata and metadata.get("cluster_project"):
            print('*********************************************************')
            print('*********************************************************')
            zone_pattern = re.compile(r'-\w$')
            location_id = metadata.get("zone")
            if zone_pattern.search(location_id):
                region = location_id.rsplit('-', 1)[0]
            else:
                region = location_id
            return metadata.get("cluster_name"), region, metadata.get("cluster_project"), metadata.get("node_pool_name"), metadata.get("accelerator_type")
...
def get_cluster_info_from_logs(dag_id, run_id, test_id, test_start_date, test_end_date):
    ...
    for e in entries:
        ...
        # 20260119 check xlml_metadata first
        if 'xlml_metadata' in payload_str:
            cluster_name, cluster_region, project_name, node_pool_name, accelerator_type = parse_cluster_metadata(payload_str)
            if (cluster_name):
                break
        elif match_gke:
    ...
    if (cluster_name and from_cred and project_name is None):
        ...
        if match:
            project_name = match.group(1)
        #        break
    return cluster_name, cluster_region, project_name, node_pool_name, accelerator_type
...
def process_test(dag_id, run_id, execution_date, test_id, test_start_date, test_end_date):
    ...
    if not test_start_date or not test_end_date:
        ...
        return cluster_name, region, project_name, node_pool_name, accelerator_type

    cluster_name, region, project_name, node_pool_name, accelerator_type = get_cluster_info_from_logs(dag_id, run_id, test_id, test_start_date, test_end_date)
...
def save():
    ...
    #query = f"SELECT * FROM `{BQ_PROJECT_ID}.{BQ_DATASET}.{BQ_VIEW_NAME_NO_CLUSTER}` where dag_id='multi-host-availability-rollback'"
    ...
    schema = [
        ...
        bigquery.SchemaField("node_pool_name", "STRING"),
        bigquery.SchemaField("accelerator_type", "STRING"),
    ]

    output_data = []
    for row in rows:
        cluster_name, region, project_name, node_pool_name, accelerator_type = process_test(
                row.dag_id, row.run_id, row.execution_date, row.test_id, row.test_start_date, row.test_end_date)
        print(f'{row.dag_id}.{row.test_id} cluster:{cluster_name},region:{region},project:{project_name},node_pool_name:{node_pool_name},accelerator_type:{accelerator_type}')
