...
from dags.dashboard.statistic.config import BQ_PROJECT_ID, BQ_DATASET, GCS_BUCKET_NAME


from kubernetes import client as k8s_client

# --- Attempt to import Airflow/Google provider modules (For the conditional logic) ---
try:
    # Use simple conditional import to check environment
    #from airflow.providers.google.cloud.hooks.container import GKEHook
...
def list_clusters_for_project(project_id):
    ...
    response = client.list_clusters(request={"parent": parent}, retry=no_retry, timeout=90)
    #response = client.list_clusters(request={"parent": parent}, retry=None)
...
def get_cluster_status(project_id, location, cluster_name):
    ...
    #cluster = client.get_cluster(request=request, retry=None)
    cluster = client.get_cluster(request=request, retry=no_retry, timeout=90)
    ...
    if cluster_mode == "Standard":
      api_client = None  
      if IS_AIRFLOW_ENV:
        print(f'IS_AIRFLOW_ENV, try get k8s api_client for cluster {project_id}.{cluster_name}....')
        api_client = get_k8s_api_client(project_id, location, cluster_name)
        print(f'api_client:{api_client}')
      for np in cluster.node_pools:
        #node_count = get_node_count_via_kubectl(np.name)
        node_count = get_node_count_via_kubectl(api_client, cluster_name, np.name)
        #node_count=None
...
def get_k8s_api_client(project_id, location, cluster_name):
    #print("  -> Using Airflow GKEHook for authentication.")
    #hook = GKEHook(gcp_conn_id=GCP_CONN_ID)
    #api_client = hook.get_kube_config_and_load_context(
    #    cluster_name=cluster_name,
    #    project_id=project_id,
    #    location=location
    #)
    #return api_client
    return gke.get_authenticated_client(
        project_id, location, cluster_name
    )


def get_node_count_via_kubectl(api_client, cluster_name, nodepool_name):
    try:
        v1 = None
        if IS_AIRFLOW_ENV:
        ...
        response = v1.list_node(label_selector=label_selector, _request_timeout=10)
        print(f'{cluster_name}.{nodepool_name} kubectl response item length:{len(response.items)}')
        return len(response.items)
    except Exception as e:
        print(f"Failed to query {cluster_name}.{nodepool_name}. Error: {e}")
        return -1


def get_node_count_via_kubectl_local(nodepool_name):
    """
