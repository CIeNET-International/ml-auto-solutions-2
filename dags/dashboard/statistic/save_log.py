...
try:
  ...
  from dags.dashboard.statistic import log_explorer_err_sentence as err_sentence
except ImportError:
...
BQ_DEST_TABLE_TASKS = "dag_runs_with_logs_tasks"
...
def process_test(row, dag_id, run_id, execution_date, test):
    ...
    for r in logs_keywords_entries:
        tasks_data.append({
            "dag_id": dag_id,
            "test_id": test_id,
            "task_id": r["task_id"],
...
def save():
    ...
    for row in rows:
        ...
        for test in row["tests"]:
            test_data, task_data = process_test(row, dag_id, run_id, execution_date, test)
            if test_data:
                tests_data.append(test_data)
            if task_data:
                tasks_data.extend(task_data)
    ...
    utils.save_to_gcs_and_load_bq(tasks_data, schema_tasks, BQ_DEST_TABLE_TASKS)
